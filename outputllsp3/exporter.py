...
``python-first``
    ...
    New in 0.33.0:
    - Boolean operators (and / or / not) render as Python boolean expressions.
    - Arithmetic: modulo (%), random.randint, round, string join/length/index.
    - Sensor expressions: orientation, timer, loudness, color, distance,
      reflected light, button, force, motor speed, motor position.
    - ``flippersensors_isInBetween`` renders as ``lo <= value <= hi``.
    - ``control_forever`` renders as ``while True:``.
    - ``control_stop`` / ``flippercontrol_stop`` renders as ``return``.
    - ``control_wait_until`` renders as a polling ``while`` loop.
    - Motor opcodes: ``run_motor``, ``stop_motor``, ``run_motor_for``,
      ``motor_go_to_position``, ``set_motor_speed``.
    - Sound opcodes: ``robot.beep``, ``robot.beep_for``, ``robot.stop_sound``.
    - Display opcodes: ``robot.show_image``, ``robot.show_text``,
      ``robot.turn_off_pixels``.
    - All ``__stmt__`` placeholders replaced: ``setMovementPair`` →
      ``robot.use_pair``, ``startDualSpeed`` → ``robot.drive``, etc.
    - Multiple ``whenProgramStarts`` stacks each become a named function.
    - ``import random`` added to the generated header.
